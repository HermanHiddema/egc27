...
The application currently reads player data through `EgdLookupService`
(`app/services/egd_lookup_service.rb`), which still calls the legacy
`GetPlayerDataByData.php` / `GetPlayerDataByPIN.php` endpoints for the
participant-registration autocomplete. Those endpoints need no token.

Use the GraphQL API described above for any new EGD integration, and keep this
file in sync with practical findings so the next integration does not repeat the
same discovery work.
